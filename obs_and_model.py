"""
Your module contains the Observations and Model classes.
"""
...
class Observation(object):
    ...
    This class handles the data that is going to be fit with the models. It includes
    the image to be fit, the associated uncertainty (optionally ), the redshift of the 
    object and the seeing.
    ...
    def info(self):
        ...
        if self.unc_path is None:
            print('No uncertainty image foud. Using uniform uncertainty.')
        else:
...
class Model(object):
    ...
    at a particular redshift) and observations (i.e. alignes the maps with the data).
    ...
    def create_displacement_maps_for_object(self,Observation,correct_z=True):
        """ Takes the more global displacement maps usually produced by a graviatational 
        lensing fitting code, and converts these maps to displacements at the redhisft of 
        the source being analysed and aligns and regrids the maps to the data.
...
class Metallicity_Gradient_with_Flatenning(Model):
